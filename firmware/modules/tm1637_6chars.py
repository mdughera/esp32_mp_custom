...
29/11/2024 Tolto tutte le sleep sincrone il display sembra comunque funzionare. Da verificare se questo risolve i problemi di crash probabilmente dati dal BLE
"""
...
class TM1637(object):
    ...
    def write(self, segments, pos=0):
        ...
        # modificato per gestire display a 6 caratteri
        for i in range(len(segments), 6):
            segments.append(0x00)
        self._write_byte(segments[2])
        self._write_byte(segments[1])
        self._write_byte(segments[0])
        self._write_byte(segments[5])
        self._write_byte(segments[4])
        self._write_byte(segments[3])
        self._stop()
        self._write_dsp_ctrl()
    ...
    def numbers(self, num1, num2, colon=True):
        ...
        num1 = max(-9, min(num1, 999))
        num2 = max(-9, min(num2, 999))
